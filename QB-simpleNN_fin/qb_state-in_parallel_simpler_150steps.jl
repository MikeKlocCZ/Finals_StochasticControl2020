...
cd("/home/michal/Documents/Projects/JuliaControl/FINALS/QB-simpleNN_fin")
...
C3 = 1.8f0*MyParameters.n_steps/50 #Float32(3.8672613833194e-06) ....enhance last 50 steps!
	...
	for d in data
		...
		if iter%1 == 0
			...
			if iter == 1 || iter%100 == 0
				png(string("Figure_",iter)) #for saving figs
				...
				writedlm(string("Epoch",iter,".txt"), epoch_output)
			end
			println("iter: ", iter)
		end
		if iter%1000 == 0
			@save string("model_state-in_parallel_",iter,".bson")  p1 re opt
		end
		println("++++++++++++++++++++++")
    	Flux.Optimise.update!(opt, ps, gs)
  	end
	@save string("model_state-in_parallel_END.bson")  p1 re opt
end
